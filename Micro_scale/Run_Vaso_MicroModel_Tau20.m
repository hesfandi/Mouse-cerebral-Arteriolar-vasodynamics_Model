...
% Time constant for mechanotransdcutiuon pathways: ??_msc = -??t / ln(1 - ??)
...
Tmax = 100;                    % Total simulation duration [s]
dtp = 0.1;                     % Time step [s]
...
V_PAs = zeros(NOC, length(BCC));         % Volume [??m??]
...
%             end
            ...
            NOx=NO1+NO2+NO3+NO4;
            NO=NOx+0.4; % Add basal NO level due to assumed background neuronal activity
...
%% Differential Equations
            ...
            u(idx.Ca_juns1,i)=u(idx.Ca_juns1,i-1)+ dt*((1e-9./(p.VOL_juns(1))).*(-I_CaV3-J_juns_cyt1-I_Ca_GlM));
...
function params = parse_inputs()
    ...
    parser.addParameter('Ca_GlR_T', 0.8e-2);
